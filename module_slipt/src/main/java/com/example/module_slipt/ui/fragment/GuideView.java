 ...
 * 1.修改了onDraw中矩形部分的左上右下 -- 才能画出target的View
 * 2.以控件的中心原点为坐标系 -- 控制引导布局的方向
 */
...
//                        guideViewParams.setMargins(offsetX - width + left, offsetY - height + top, width - left - offsetX, height - top - offsetY);
                        guideViewParams.setMargins(0, 0, 0,height - top - offsetY);
        ...
        if (myShape != null) {
            ...
            switch (myShape) {
                ...
                case RECTANGULAR://圆角矩形
                    //RectF对象
                    oval.left = center[0] - PxUtils.dp2px(mContent,94) /2;                              //左边
                    oval.top = center[1] - PxUtils.dp2px(mContent,142) /2;                                   //上边
                    oval.right = center[0] +  PxUtils.dp2px(mContent,94) /2;                             //右边
                    oval.bottom = center[1] + PxUtils.dp2px(mContent,142) /2;                                //下边
